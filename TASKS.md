...
| 2 | Output: shim as PID 1, output files, `logs -f` via SSE relay, cursors | 🚧 in progress |
| 3 | Snapshots + blob store, stop/resume same host and cross host, epochs/fencing — the migration test | ⏳ |
| 4 | Steering + adapters: `POST /input`, `lux-fake` over ACP, `acp` adapter, `claude-code` | ⏳ |
| 5 | S3 (straight through luxd), presigned downloads, host-local GC, retention | ⏳ |
