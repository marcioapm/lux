...
- Dense and calm. 13px body, 32px table rows (28px dense), hairline borders,
  one elevated surface (the card). Nothing animates except live state dots,
  spinners and toasts.
- Ids, hosts, logs and numeric columns are monospace with tabular numerals.
  Large standalone numbers (stat tiles) use proportional figures.
...
| Chart | `--chart-1` … `--chart-8` (fixed order), `--chart-grid` `--chart-axis` `--chart-label` `--chart-cursor` |
| Logs | `--log-stderr-bg` `--log-stderr-fg` `--log-line-hover` |
| Type | `--font-sans` `--font-mono`, `--text-{xs,sm,md,lg,xl,2xl,3xl}` (11–28px), `--leading-{tight,normal}`, `--weight-{normal,medium,semibold}` |
| Spacing | `--sp-1` … `--sp-9` (2, 4, 6, 8, 12, 16, 24, 32, 48px) |
...
| Layout | `--sidebar-w` `--topbar-h` `--row-h` `--control-h` `--control-h-sm` `--control-h-lg` |
...
optional vertical `marks`), Timeline (placement waterfall), LogView, KeyValue,
IdChip, Code, ConfirmDialog, Dialog (a form modal), Toast (`useToast`),
EmptyState, Spinner, Skeleton. All exported from `src/ds/index.ts` with typed
props.
