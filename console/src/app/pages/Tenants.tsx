...
import { Card, formatBytes, formatCount, formatRelative, formatTimestamp, IdChip, Table, Tooltip, type Column } from "../../ds/index.ts";
import { api, useNow, useQuery, type Tenant } from "../../api/index.ts";
...
export function Tenants() {
  ...
  const cols = useMemo<Column<Tenant>[]>(
    () => [
      ...
      { key: "created", header: "Created", cell: (t) => <Tooltip content={formatTimestamp(t.createdAt)}><span>{formatRelative(t.createdAt, now)}</span></Tooltip>, sortValue: (t) => Date.parse(t.createdAt), align: "right", width: 96 },
    ],
    [now],
  );
